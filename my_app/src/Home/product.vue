<template>
  <div>
        <section class="product">
            ...
            <div class="product-list" v-for="item of arr" :key="item.name">
                    <div class="product-item product-item-horizontal">
                        <a href="/product/9010966.html?huaPid=homepage-f101" class="navigation">
                            <div class="product-item-pic"><img  v-bind:src="item.image_url" /></div>
                            <div class="product-item-info">
                                <p class="product-item-info-name text-overflow">{{ item.name }}</p>
                                <!--Dev:: text-overflow Or text-overflow-line2-->
                                <!-- 限时抢购标签 tag-promo，同商品列表页  -->
                                <p class="product-item-info-desc text-overflow-line2 ">{{item.product_desc}}</p>
                                <div class="product-item-info-promo">{{item.product_promo}}</div>
                                <div class="product-item-info-bottom">
                                    <div class="product-item-info-bottom-left">
                                        <p class="product-item-info-prices" data-id="9010966"><strong>¥{{item.price}}</strong><s>¥{{item.market_price}}</s></p>
                                        <p class="product-item-info-sales">{{item.product_tag}}</p>
                                    </div>
                                    <div class="product-item-info-bottom-right"><i class="iconfont icon-cart-full"></i></div>
                                </div>
                            </div>
                        </a>
                    </div>
                ...
                <router-link to="" class="product-more-btn">查看更多</router-link>
            </div>
...
<script>
import data from "../../public/shuju.json";
export default {
 data() {
    return {
      arr: []
...
.product-item > .navigation {
  overflow: hidden;
  -webkit-box-shadow: 0 0.2rem 0.4rem 0 #e9ecf0;
  box-shadow: 0 0.28571429rem 0.4rem 0 #e9ecf0;
